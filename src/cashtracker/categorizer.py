...
class QuitInteractive(Exception):
    """Raised when the user quits interactive categorization early."""
...
# Receives (transaction, ai_suggestion or None, list of category names, current index, total uncategorized).
# Returns (chosen_category, keyword_to_learn) or None to skip.
PromptFn = Callable[[Transaction, Optional[str], list[str], int, int], Optional[tuple[str, str]]]


# Callback for saving keywords incrementally
SaveFn = Callable[[dict[str, list[str]]], None] | None

# Callback for reporting interactive status (total_transactions, auto_categorized)
StatusFn = Callable[[int, int], None] | None
...
def categorize_transactions(
    ...
    if interactive and prompt_fn:
        ...
        auto_categorized = len(transactions) - total_unmatched
        if status_fn:
            status_fn(len(transactions), auto_categorized)
        current_idx = 0
        try:
            while still_unmatched:
                ...
                choice = prompt_fn(txn, ai_suggestion, category_names, current_idx, total_unmatched)
                if choice:
            ...
            # Mark remaining as uncategorized
            for remaining_txn in still_unmatched:
                remaining_txn.category = "uncategorized"
                remaining_txn.confidence = 0.0
    elif use_ai:
