...
DEFAULT_PERSONAL_CONFIG_NAME = "personal.yaml"
...
class PersonalConfig:
    """Optional local-only statement cleanup settings."""

    cardholder_names: list[str] = field(default_factory=list)


def load_personal_config(path: Path | None = None) -> PersonalConfig:
    """Load ignored, machine-local statement cleanup settings.

    A missing file is expected and produces an empty configuration.
    """
    if path is None:
        path = Path(DEFAULT_PERSONAL_CONFIG_NAME)
    ...
    statement_noise = raw.get("statement_noise", {})
    if statement_noise is None:
        statement_noise = {}
    if not isinstance(statement_noise, dict):
        raise ValueError("'statement_noise' must be a mapping")

    names = statement_noise.get("cardholder_names", [])
    if names is None:
        names = []
    if not isinstance(names, list):
        raise ValueError("'cardholder_names' must be a list")

    return PersonalConfig(cardholder_names=[str(name).strip() for name in names if str(name).strip()])
