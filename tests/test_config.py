...
class TestLoadPersonalConfig:
    def test_missing_file_is_empty(self, tmp_path: Path):
        cfg = load_personal_config(tmp_path / "missing.yaml")
        assert cfg.cardholder_names == []

    def test_loads_cardholder_names(self, tmp_path: Path):
        path = tmp_path / "personal.yaml"
        path.write_text(yaml.dump({
            "statement_noise": {"cardholder_names": ["Jane Q Cardholder"]}
        }))
        cfg = load_personal_config(path)
        assert cfg.cardholder_names == ["Jane Q Cardholder"]
